...
function L_condition_U(L, M, P)
	if (abs(L) <= par.π_star)
		U = (-1/(2*P[1,3]))*(2*(P[1,1]* L + P[1,2]*M ) - L - ϵ)
	elseif L > par.π_star
		U = (-1/(2*P[1,3]))*(2*(P[1,1]* L + P[1,2]*M ) - par.α*(L - par.π_star) - L - ϵ)
	elseif L < par.π_star
		U = (-1/(2*P[1,3]))*(2*(P[1,1]* L + P[1,2]*M ) - par.α*(L + par.π_star) - L - ϵ)
	end
	return U
...
function U_condition_L(M, U, P)
	if (abs(U) <= par.π_star)
		L = (-1/(2*P[3,1]))*(2*(P[3,3]* U + P[3,2]*M ) - U + ϵ)
	elseif U > par.π_star
		L = (-1/(2*P[3,1]))*(2*(P[3,3]* U + P[3,2]*M ) - par.α*(U - par.π_star) - U + ϵ)
	elseif U < par.π_star
		L = (-1/(2*P[3,1]))*(2*(P[3,3]* U + P[3,2]*M ) - par.α*(U + par.π_star) - U + ϵ)
	end
	return L
...
function U_condition_M(L, U, P)
	if (abs(U) <= par.π_star)
		M = (-1/(2*P[3,2]))*(2*(P[3,3]* U + P[3,1]*L ) - U + ϵ)
	elseif U > par.π_star
		M = (-1/(2*P[3,2]))*(2*(P[3,3]* U + P[3,1]*L ) - par.α*(U - par.π_star) - U + ϵ)
	elseif U < par.π_star
		M = (-1/(2*P[3,2]))*(2*(P[3,3]* U + P[3,1]*L ) - par.α*(U + par.π_star) - U + ϵ)
	end
	return M
...
solns = comb_conditions[(abs.(comb_conditions.L .- comb_conditions.L_soln) .< 0.1) .*
						(abs.(comb_conditions.M .- comb_conditions.M_soln) .< 0.1) .*
						(abs.(comb_conditions.U .- comb_conditions.U_soln) .< 0.1) ,:]

plot(xlabel = L"\epsilon_t", ylabel = L"\pi_t")
plot!([-ϵ, 0, ϵ], collect(solns[1,[:L, :M, :U]]), color = 1, label = "")
scatter!([-ϵ, 0, ϵ], collect(solns[1,[:L, :M, :U]]), color = 1, label = "")
plot!([-ϵ, 0, ϵ], collect(solns[2,[:L, :M, :U]]), color = 2, label = "")
scatter!([-ϵ, 0, ϵ], collect(solns[2,[:L, :M, :U]]), color = 2, label = "")
plot!([-ϵ, 0, ϵ], collect(solns[3,[:L, :M, :U]]), color = 3, label = "")
scatter!([-ϵ, 0, ϵ], collect(solns[3,[:L, :M, :U]]), color = 3, label = "")
plot!(zeros(2), [minimum(solns.L), maximum(solns.U)], color = :black, linestyle = :dot, label = "")
plot!([-ϵ, ϵ], zeros(2), color = :black, linestyle = :dot, label = "")
plot!(size = (300,300))
...
plot(Ms, Us, unstack(L_htmp, :M, :U, :L_soln), st=:surface, camera = (30,30))
plot(M_htmp.L, M_htmp.M_soln, M_htmp.U, color = :purple)
plot(U_htmp.L, U_htmp.M, U_htmp.U_soln, color = :red)
plot!(camera = (40, 50))
M_solns = DataFrame(zeros(length(Us)*length(Ls), 3), [:U, :L, :M_soln])
ii = 1
for u in Us
	for l in Ls
		M_soln[ii] = U_condition_M(l,u,P)
		ii+=1
	end
end
U_soln = zeros(length(Ms)*length(Ls))
ii = 1
for u in Us
	for m in Ms
		L_soln[ii] = M_condition_L(m,u,P)
		ii+=1
	end
end


plot(xlabel = L"L = \pi_t | ϵ_t = -\epsilon",
	ylabel = L"U = \pi_t | ϵ_t = \epsilon",
	zlabel = L"M = \pi_t | ϵ_t = 0",
	legend=:topleft )


hcat([(collect(x)) for x in vec(collect(Iterators.product(Ms, Us)))]...)
Iterators.product(Us, Ms)

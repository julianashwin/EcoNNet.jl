...
@everywhere options = EcoNNetOptions(infoset = [:π_lag, :y_lag, :y3_lag, :ϵ_π, :ϵ_y],
...
plot!(size = (600,300))
if save_figs
	savefig("figures/illus_rls_sim_series.pdf")
end
...
plot_ss(plot_points)
initial_ss = deepcopy(central)
starts = [(π=3.0,y=3.0,periods=150,arrows=[10]),
	(π=-3.,y=-3.,periods=150,arrows=[10]),
	(π=0.01,y=0.01,periods=150,arrows=[80, 95]),
	(π=-0.01,y=-0.01,periods=150,arrows=[80, 95]),
...
gif(anim, "figures/phase_illus_rls_sim.gif", fps = 15)
end
...
upper_stoch[:y] = paths.y[1000];upper_stoch[:y3] = paths.y3[1000];
...
lower_stoch[:y] = paths.y[1000];lower_stoch[:y3] = paths.y3[1000];
...
if save_figs
	plot!(size=(300,200))
	savefig("figures/irf1_illus_rls_sim.pdf")
end
...
if save_figs
	plot!(size=(300,200))
	savefig("figures/irf2_illus_rls_sim.pdf")
end
...
if save_figs
	plot!(size=(300,200))
	savefig("figures/irf3_illus_rls_sim.pdf")
end
...
if save_figs
	plot!(size=(300,200))
	savefig("figures/irf4_illus_rls_sim.pdf")
end
...
if save_figs
	plot!(size=(600,400))
	display(irf_plot)
	savefig("figures/irfs_illus_rls_sim.pdf")
end
...
Compare PLM and ALM
"""
# Simulate the system for many  periods without updating beliefs
s_final = deepcopy(s)
noise_π = par.σ_π*randn(nrow(s_final))
s_final.ϵ_π = simulate_ar(par.ρ_π, par.σ_π, options.N, noise_π)
noise_y = par.σ_y*randn(nrow(s_final))
s_final.ϵ_y = simulate_ar(par.ρ_y, par.σ_y, options.N, noise_y)
plot(s_final.ϵ_y[1:200])

options.learning_gap = 2*options.N
@time beliefs,s_final = simulate_learning(2:options.N, s_final, beliefs, indices, options)

s_final.Eπ_error = s_final.Eπ-s_final.π
s_final.Ey_error = s_final.Ey-s_final.y
s_final.Eπ_lead_error = vcat((s_final.Eπ_lead[1:(options.N-1)]-s_final.π[2:(options.N)]),[0.])
plot(s_final.Eπ_lead_error)

plot_range=2:500000
y_range = Array(-6.0:0.1:6.0)
π_range= Array(-3.5:0.1:3.5)
heatmap_df = deepcopy(s_final[plot_range,:])
heatmap_df.y_lag = heatmap_df[plot_range.-1,:y]
...
if save_figs
	plot!(size=(800,300))
	savefig("figures/heatmap_illus_rls_sim.pdf")
end


avg_error = zeros(len(y_range))
for yy in 1:len(y_range)
	instances = Array(heatmap_df[:,:y_grid].==y_range[yy])
	errors = heatmap_df[instances,:Eπ_lead_error]
